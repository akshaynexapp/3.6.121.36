...
def get_settings_value(option, default):
    ...
    if outdated_option_used:
        logger.warn(
            ...
            f'in the future, please use Nexapp_NETWORK_TOPOLOGY_{option} instead.'
        )
        return getattr(settings, f'{outdated_option_used}_{option}')
    return getattr(settings, f'Nexapp_NETWORK_TOPOLOGY_{option}', default)
...
TOPOLOGY_API_AUTH_REQUIRED = get_settings_value('API_AUTH_REQUIRED', True)
